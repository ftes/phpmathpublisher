...
class Helper
{
	...
	public function tableau_expression($expression)
	{
		...
		{
			$result[] = new expression_texte($extraits[$i], $this);
		}
		return $result;
	}
	...
	{
		$symboles = $this->getSymboles();
		$fontesmath = $this->getFontesmath();
		...
		$fontG = $this->getFontG();
		$fontB = $this->getFontB();
		$transperent = $this->getTransperent();
		;
	...
	public function setFontesmath($fontesmath)
	{
		$this->fontesmath = $fontesmath;
	}

	public function getFontesmath()
	{
		return $this->fontesmath;
	}

	public function setSymboles($symboles)
	{
	...
	public function setTransperent($transperent)
	{
		$this->transperent = $transperent;
	}

	public function getTransperent()
	{
...
//	public function expression_texte($exp)
//	{
//		$this->texte = $exp;
//	}
	...
	{
		$this->image = $this->helper->affiche_math($this->texte, $taille);
...
//	}
		...
		for($i = 0; $i < count($this->noeuds); $i++)
		{
			...
			elseif($this->noeuds[$i]->texte == ')' || $this->noeuds[$i]->texte == '}')
			{
				...
				if(count($parentheses) == 0)
				{
					...
					{
						$ret[] = new expression_math(array(new expression_texte("(", $this->helper), new expression_math($sub, $this->helper), new expression_texte(")", $this->helper)), $this->helper);
					}
					else $ret[] = new expression_math($sub, $this->helper);
				}
			}
	...
	public function traite_operation($noeuds, $operation)
	{
		do
		{
			...
			for($i = 0; $i < count($noeuds); $i++)
			{
				...
				{
					$ret[] = new expression_math(array($noeuds[$i], $noeuds[$i + 1], $noeuds[$i + 2]), $this->helper);
					$i += 2;
					$change = true;
				}
				else
	...
	public function traite_fonction($noeuds, $fonction, $nbarg)
	{
		...
		for($i = 0; $i < count($noeuds); $i++)
		{
			if($i < count($noeuds) - $nbarg && $noeuds[$i]->texte == $fonction)
			{
				...
				$ret[] = new expression_math($a, $this->helper);
				$i += $nbarg;
			}
			else
				$ret[] = $noeuds[$i];
	...
	public function dessine_expression($taille)
	{
		...
		$hauteur = $dessus + $dessous;
		$paro = $this->helper->parenthese(max($dessus, $dessous) * 2, "(");
		$parf = $this->helper->parenthese(max($dessus, $dessous) * 2, ")");
		for($i = 0; $i < count($this->noeuds); $i++)
		{
			if(!isset($img[$i]))
			{
	...
	public function dessine_delimiteur($taille)
	{
		...
		if($this->noeuds[1]->texte == "&$")
			$imggauche = $this->helper->parenthese($hauteurexp, $this->noeuds[1]->noeuds[0]->texte);
		else $imggauche = $this->helper->parenthese($hauteurexp, $this->noeuds[1]->texte);
...
class PMP_PhpMathPublisher
{
	...
	public function mathimage($text, $size, $pathtoimg)
	{
		...
		if($v == 0)
		{
			...
			$formula = new expression_math($this->helper->tableau_expression(trim($text)), $this->helper);
